...
from keras.layers import Conv1D, Embedding, Concatenate, Dropout
...
print("Second largest sequence length: %s" % seconde_largest_length_cp)
...
#true_label = open('MultichannelCLSTM_true_label.txt', 'w')
...
for fsz in filter_sizes:
    ...
    lstm_ab.append(l_lstm)

attention_input_ab = Lambda(lambda x: K.stack([x[0], x[1], x[2]], axis = 1))([lstm_ab[0], lstm_ab[1], lstm_ab[2]])
# self-attention on biLSTM 
l_attention_ab = Lambda(selfAttention, output_shape = lambda input_shape:input_shape)(attention_input_ab)
l_flat_ab = Flatten()(l_attention_ab)
...
lstm_cp = []
for fsz in filter_sizes:
    l_conv_cp = Conv1D(nb_filter=128,filter_length=fsz,activation='relu')(embedded_sequences_cp)
    print(l_conv_cp.shape)
    #l_norm = BatchNormalization()(l_conv)
    # biLSTM on feature map
    l_lstm_cp = LSTM(128, activation='tanh', return_sequences = False)(l_conv_cp)
    lstm_cp.append(l_lstm_cp)

attention_input_cp = Lambda(lambda x: K.stack([x[0], x[1], x[2]], axis = 1))([lstm_cp[0], lstm_cp[1], lstm_cp[2]])
# self-attention on biLSTM 
l_attention_cp = Lambda(selfAttention, output_shape = lambda input_shape:input_shape)(attention_input_cp)
l_flat_cp = Flatten()(l_attention_cp)

l_merged = Concatenate(axis = 1)([l_flat_ab, l_flat_cp])

l_fc = Dense(256, activation='relu')(l_merged)
...
print("model fitting - more complex CLSTM")
model.summary()
...
model.fit_generator(generator = data_generator_two(x_train_ab, x_train_cp, y_train, batch_size = BATCH_SIZE, 
                    ...
                    steps_per_epoch = len(x_train_ab) // BATCH_SIZE, epochs = 5, 
...
#with open("MultichannelCLSTM_model.json", "w") as json_file:
#    json_file.write(model_json)
# serialize weights to HDF5
#model.save_weights("MultichannelCLSTM_model_weights.h5")
...
#pred_label_5 = open('MultichannelCLSTM_pred_label_5.txt', 'w')
#for meshs in top_5_mesh:
#    mesh = ' '.join(meshs)
#    pred_label_5.writelines(mesh.strip()+ "\r")
#pred_label_5.close()

#pred_label_10 = open('MultichannelCLSTM_pred_label_10.txt', 'w')
