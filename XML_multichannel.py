...
# -*- coding: utf-8 -*-
"""
Created on Sat Jan  5 23:29:23 2019
...
from keras.layers import Conv2D, Embedding, MaxPooling2D, Concatenate, Dropout, BatchNormalization, Reshape
...
true_label = open('MultichannelXML_true_label.txt', 'w')
...
reshape_ab = Reshape((MAX_SEQUENCE_LENGTH_ab, EMBEDDING_DIM, 1))(embedded_sequences_ab)
for fsz in filter_sizes:
    l_conv_XML = Conv2D(nb_filter=128,kernel_size=(MAX_SEQUENCE_LENGTH_ab, fsz),activation='relu', data_format = 'channels_last')(reshape_ab)
...
#    pool_size = (EMBEDDING_DIM - fsz + 1, 1)
    ...
    convs_XML_ab.append(l_pool_XML)
l_merge_XML_ab = Concatenate(axis=2)(convs_XML_ab)
...
reshape_cp = Reshape((MAX_SEQUENCE_LENGTH_cp, EMBEDDING_DIM, 1))(embedded_sequences_cp)
for fsz in filter_sizes:
    l_conv_XML = Conv2D(nb_filter=128,kernel_size=(MAX_SEQUENCE_LENGTH_cp, fsz),activation='relu', data_format = 'channels_last')(reshape_cp)
    l_norm = BatchNormalization()(l_conv_XML)
#    pool_size = (EMBEDDING_DIM - fsz + 1, 1)
    pool_size = (10, 1)
    l_pool_XML = MaxPooling2D(pool_size = pool_size, strides = (10, 1), padding='valid', data_format = 'channels_first')(l_norm)
    convs_XML_cp.append(l_pool_XML)
l_merge_XML_cp = Concatenate(axis=2)(convs_XML_cp)

# merge
l_merge_XML = Concatenate(axis=2)([l_merge_XML_ab, l_merge_XML_cp])
l_flat_XML = Flatten()(l_merge_XML)

l_dense = Dense(4096, activation='relu')(l_flat_XML)
...
pred_label_5 = open('MultichannelXML_pred_label_5.txt', 'w')
for meshs in top_5_mesh:
    mesh = ' '.join(meshs)
    pred_label_5.writelines(mesh.strip()+ "\r")
pred_label_5.close()

pred_label_10 = open('MultichannelXML_pred_label_10.txt', 'w')
for meshs in top_10_mesh:
    mesh = ' '.join(meshs)
    pred_label_10.writelines(mesh.strip()+ "\r")
pred_label_10.close()

pred_label_15 = open('MultichannelXML_pred_label_15.txt', 'w')
