...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct  1 23:26:58 2018
...
from keras.layers import Conv1D, MaxPooling1D, Embedding, Concatenate, Dropout, BatchNormalization, Reshape, Conv2D, MaxPooling2D
...
for fsz in filter_sizes:
    l_conv_XML = Conv2D(nb_filter=128,
                        kernel_size=(MAX_SEQUENCE_LENGTH, fsz),
                        activation='relu', data_format = 'channels_last')(reshape)
    l_norm = BatchNormalization()(l_conv_XML)
    pool_size = (10, 1)
    l_pool_XML = MaxPooling2D(pool_size = pool_size, 
                              strides = (10, 1), 
                              padding='valid', 
                              data_format = 'channels_first')(l_norm)
    convs_XML.append(l_pool_XML)
l_merge_XML = Concatenate(axis=2)(convs_XML)
l_flat_XML = Flatten()(l_merge_XML)

# stack feature maps extracted from CNN and XML
featureMap = [l_flat,  l_flat_XML]
l_feature = Concatenate()(featureMap)

l_dense = Dense(512, activation='relu')(l_feature)
l_norm2 = BatchNormalization()(l_dense)
dense_out = Dropout(0.5)(l_norm2)
